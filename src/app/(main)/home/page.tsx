  ...
  return (
    <div className="space-y-5 stagger-children max-w-2xl mx-auto">
      ...
      <div className="animate-slide-up">
        <h1 className="text-xl font-semibold text-[#F4EED9]">{t('chooseMode')}</h1>
        <div className="flex items-center gap-1.5 mt-1">
          <span className="w-2 h-2 rounded-full bg-[#8B6D3B] animate-pulse-glow" />
          <span className="text-sm text-[#84796B]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 animate-slide-up">
        <button
          ...
          className={`mode-card mori-card p-5 text-left ${mode === 'anonymous' ? 'selected' : ''}`}
        >
          <div className="flex items-start gap-3">
            <div className="w-9 h-9 rounded-lg bg-[#26231d] flex items-center justify-center flex-shrink-0">
              <EyeOff size={18} className="text-[#84796B]" />
            </div>
            <div>
              <h3 className="font-medium text-sm text-[#F4EED9]">{t('anonymousMode')}</h3>
              <p className="text-xs text-[#84796B] mt-0.5 leading-relaxed">{t('anonymousModeDesc')}</p>
            </div>
          </div>
          {mode === 'anonymous' && (
            <div className="absolute top-3 right-3">
              <div className="w-5 h-5 rounded-full bg-[#8B6D3B] flex items-center justify-center animate-scale-in">
                <Check size={12} className="text-[#1c1a16]" />
              </div>
            </div>
          )}
        </button>
        ...
        <button
          ...
          className={`mode-card mori-card p-5 text-left ${mode === 'revealed' ? 'selected' : ''}`}
        >
          <div className="flex items-start gap-3">
            <div className="w-9 h-9 rounded-lg bg-[#26231d] flex items-center justify-center flex-shrink-0">
              <Eye size={18} className="text-[#84796B]" />
            </div>
            <div>
              <h3 className="font-medium text-sm text-[#F4EED9]">{t('revealedMode')}</h3>
              <p className="text-xs text-[#84796B] mt-0.5 leading-relaxed">{t('revealedModeDesc')}</p>
            </div>
          </div>
          {mode === 'revealed' && (
            <div className="absolute top-3 right-3">
              <div className="w-5 h-5 rounded-full bg-[#8B6D3B] flex items-center justify-center">
                <Check size={12} className="text-[#1c1a16]" />
              </div>
            </div>
          )}
      ...
      {/* Interest Tags */}
      <div className="mori-card p-5 animate-slide-up">
        <TagSelector
      ...
      <div className="grid grid-cols-2 gap-3 animate-slide-up">
        <div className="mori-card p-4 text-center">
          <p className="text-xl font-semibold text-[#F4EED9]">{userProfile?.totalChats || 0}</p>
          <p className="text-xs text-[#84796B] mt-0.5">{t('totalChats')}</p>
        </div>
        <div className="mori-card p-4 text-center">
          <p className="text-xl font-semibold text-[#F4EED9]">{selectedTags.length}</p>
          <p className="text-xs text-[#84796B] mt-0.5">{t('interests')}</p>
        </div>
      </div>
